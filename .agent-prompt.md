...
<<<<<<< HEAD
Build spec — folder-track-header (kit composite; read KIT-LEAD.md). The group/submix variant of the (built) TrackHeader. Grounded in the settled grouping decision: native Tracktion submix folders — a parent Folder track sums its children and applies a group fader + group plugin chain.

What it is. A track-header row for a Folder track, visually distinct from a normal track header: wider keyline + recessed background (reads as a container/parent), a disclosure control (collapse/expand its children), a group fader + group pan (the submix controls), and the FX chip (group plugin chain). No arm, no input — a folder doesn't record; it sums children. Meter = the group's summed level (ears-first, same rules as TrackHeader).

Props / contract. Folder TrackInfo (kind:'folder', parentId?, child count) + mixer (group db/pan/mute/solo). Callbacks: collapse/expand (UI view state — localStorage), track.setVolume/setPan/setMute/setSolo (the group), track.rename, FX chip → plugin.*. Dogfood the kit primitives (Fader, PanKnob, MuteSoloToggle, FxChip, Meter) — don't rebuild them.

States (all in the gallery). collapsed / expanded, with / without children, focused-selected, group muted / soloed, hover, disabled. Tokens only; Compare light AND dark; reduced-motion (the disclosure transition is decorative).

Done. A distinct folder/group header (wider keyline + recessed bg, disclosure, group fader + pan, FX chip, no arm/input, summed meter) composing the kit primitives; all states in the gallery; themeable; tsc --noEmit + vitest run + lint green.
=======
Build spec — tooltip (kit primitive; read KIT-LEAD.md). The hover/focus hint. Composes the shared Popover shell (anchorRef mode) — don't reinvent positioning.

What it is. A small label/hint that appears on hover or keyboard focus of its trigger, on a compact dark --stage surface with on-stage (light) text + a hairline (small arrow optional). Portals into the themed mount (usePortalTarget()); viewport-flips via Popover.

Behavior. Show after a short delay (~400–600 ms) on hover; show immediately on focus; hide on blur / mouse-leave / Esc / scroll. One at a time. No tooltip on touch.

Props. content, the trigger (children + anchorRef), placement?, delay?, disabled?. Display-only.

a11y. role="tooltip", linked via aria-describedby; focus-triggered (not just hover) so keyboard users get it; never trap focus; Esc dismisses.

Chroma/motion. Quick fade/scale in (≤150 ms), tokens; decorative → snaps under reduced-motion. Compare light + dark.

States/demo. hidden, visible (hover), visible (focus), all placements, long content (wraps), disabled. Dogfood: attach to kit buttons.

Done. A small accessible tooltip on the Popover shell, hover+focus with delay, themed dark surface, viewport-flipping; tsc + vitest + lint green.
>>>>>>> kit/tooltip
