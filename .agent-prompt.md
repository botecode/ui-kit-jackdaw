...
Build spec — shortcuts (kit composite; read KIT-LEAD.md). Preferences > Shortcuts panel. Depends on text-field. Reference: the Shortcuts screenshot.

Left. A search (text-field) + a Sort select (Category…) + a scrollable list of actions — each: name, category ("Clip" / "Clips"), the current bindings (⌘C, Ctrl+C…), and a Rebind button.

Right. A "new custom action" builder — a Name field, Steps ("Add action…" dropdown, chained top → bottom), an Assign key button, and Save custom action.

Props / contract. actions: { id, name, category, bindings[] }[]; callbacks → rebind(id), search/sort (UI state), createMacro(name, steps, key). Rebind/Assign-key = capture a live keystroke (reuse the keybinding registry from clip-glue if it's landed; don't reinvent).

States. list, searching, sorted, a rebind in progress (capturing), the builder (empty / with steps / assigning key). Tokens; Compare light + dark; a11y.

Done. A searchable shortcuts list with rebind + a custom-action (macro) builder; all states; tsc + vitest + lint green.
